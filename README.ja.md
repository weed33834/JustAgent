# JustAgent：監査可能なマルチエージェントプラットフォーム

[English](README.md) | [中文](README.zh.md) | [日本語](README.ja.md)

> **すべての操作に権限管理、すべての変更にチェックポイント、すべてのステップを監査可能に。**

JustAgent は監査可能なマルチエージェントプラットフォームで、単一の CLI ツールとオプションの Web コンソールとして提供されます。反復的なツール呼び出しループを実行し、LLM がアクションを提案し、ツールで実行して結果を検証します。すべての破壊的操作は権限エンジンを経由し、すべてのファイル変更はチェックポイント化され、すべての会話は保存・再開できます — エージェントの行動はすべてレビュー可能かつ元に戻せます。このエンジンの上に、最初のバンドル垂直アプリ **JustAgent Legal**（法務）を同梱：司法・法務チーム向けの事件資料整理、証拠審査、法的文書生成。オンプレミス LLM 対応（データは外部に出ません）。
...
1. **Agent モード** — インタラクティブ REPL またはワンショットタスク。まず計画（読み取り専用分析）、次に実行（権限プロンプト付き）、または Yolo モードでツール呼び出しを自動承認。複数ターンの会話はセッション間で永続化されます。
2. **Pipeline モード** — オプションの `clean → verify → commit → upload` ショートカット。リリースフロー用。
3. **Project モード** — 複数のプロジェクトを管理、クロスプロジェクト操作を実行。

## バンドル垂直アプリ：JustAgent Legal

法務バーティカルはエンジンを司法ワークベンチに変えます：
...
| `justagent project` | 複数のプロジェクトを管理 |
