...
- ``on_error``: 当 ``myagent verify --fix`` 失败时给出可操作建议。
"""
...
from myagent_sdk import CommandContext, FixSuggestion, Plugin, VerifyError, hook
    ...
    def on_error(self, context: CommandContext, error: Exception) -> FixSuggestion | None:
        ...
        if isinstance(error, VerifyError):
            return FixSuggestion(
                description="[project-guard] Run `myagent clean` and then re-run `myagent verify`.",
            )
