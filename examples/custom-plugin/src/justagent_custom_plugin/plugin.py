...
- ``on_error``: 当 ``justagent verify --fix`` 失败时给出可操作建议。
"""
...
from justagent_sdk import CommandContext, FixSuggestion, Plugin, VerifyError, hook
    ...
    def on_error(self, context: CommandContext, error: Exception) -> FixSuggestion | None:
        ...
        if isinstance(error, VerifyError):
            return FixSuggestion(
                description="[project-guard] Run `justagent clean` and then re-run `justagent verify`.",
            )
