"""JustAgent-CLI language pack for Rust projects.
...
from justagent_sdk import CommandContext, Plugin, hook
...
class RustShipPlugin(Plugin):
    ...
    def pre_clean(self, context: CommandContext) -> None:
        """Log Rust build artifacts present before ``justagent clean`` runs.
...
def register() -> RustShipPlugin:
    """Entry-point factory used by the ``justagent.plugins`` group."""
