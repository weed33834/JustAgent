...
issues that would only surface when a user runs ``justagent <cmd> --help``.
"""
...
from justagent.cli.main import _KNOWN_COMMANDS, app
...
def test_subcommand_help_exits_zero(command: str) -> None:
    """``justagent <command> --help`` exits 0 with no traceback."""
...
def test_top_level_help_exits_zero() -> None:
    """``justagent --help`` itself exits 0 with no traceback."""
