...
from justagent.cli.commands import fix as fix_module
...
def test_fix_calls_llm_and_shows_suggestion(tmp_path: Path, monkeypatch) -> None:
    ...
    with patch("justagent.cli.commands.fix._model_router", return_value=mock_router):
        result = runner.invoke(app, ["fix", str(error_log)])
...
def test_fix_yes_flag_applies_patch(tmp_path: Path, monkeypatch) -> None:
    ...
    with (
        patch("justagent.cli.commands.fix._model_router", return_value=mock_router),
        patch("justagent.cli.commands.fix._apply_patch") as mock_apply,
    ):
