...
def test_registry_dashboard_shows_metrics() -> None:
    ...
    with patch("justagent.cli.commands.registry.RegistryIndex") as mock_index:
...
def test_registry_sync_writes_files(tmp_path: Path) -> None:
    ...
    with (
        patch(
            "justagent.cli.commands.registry.RegistryClient.fetch_index",
            return_value=remote_data,
        ) as mock_fetch,
        patch("justagent.cli.commands.registry.BUNDLED_REGISTRY_PATH", bundled),
    ):
...
def test_registry_mirror_dry_run_does_not_write(tmp_path: Path) -> None:
    ...
    with patch("justagent.cli.commands.registry.RegistryClient") as mock_client:
...
def test_registry_mirror_writes_files(tmp_path: Path) -> None:
    ...
    with patch("justagent.cli.commands.registry.RegistryClient") as mock_client_cls:
