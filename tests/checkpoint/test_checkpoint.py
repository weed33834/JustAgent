...
from autoship.checkpoint import (
    Checkpoint,
    CheckpointConfig,
    CheckpointError,
    CheckpointManager,
...
class TestCheckpointConfig:
    def test_defaults(self) -> None:
        ...
        assert cfg.checkpoint_dir == ".autoship/checkpoints"
        assert cfg.max_checkpoints == 100
        assert ".git/" in cfg.exclude_patterns
        assert ".autoship/" in cfg.exclude_patterns
...
class TestInitialize:
    def test_initialize_creates_shadow_repo(self, project_dir: Path) -> None:
        ...
        shadow = project_dir / ".autoship" / "checkpoints"
    ...
    def test_initialize_writes_gitignore(self, project_dir: Path) -> None:
        ...
        exclude_file = (
            project_dir / ".autoship" / "checkpoints" / ".git" / "info" / "exclude"
        )
        assert exclude_file.exists()
        content = exclude_file.read_text(encoding="utf-8")
        assert ".git/" in content
        assert ".autoship/" in content
    ...
    def test_initialize_noop_when_disabled(self, project_dir: Path) -> None:
        ...
        assert not (project_dir / ".autoship").exists()
...
class TestRestore:
    ...
    def test_restore_preserves_git_and_autoship_dirs(
        self, manager: CheckpointManager, project_dir: Path
    ) -> None:
        cp = manager.snapshot(iteration=0, message="initial")
        assert cp is not None
        # The .autoship dir (shadow repo) must survive restore.
        manager.restore(cp.id)
        assert (project_dir / ".autoship").exists()
        assert (project_dir / ".autoship" / "checkpoints").exists()
...
class TestProperties:
    ...
    def test_shadow_repo_path(self, project_dir: Path) -> None:
        mgr = CheckpointManager(project_dir)
        assert mgr.shadow_repo_path == project_dir / ".autoship" / "checkpoints"
...
class TestExcludePatterns:
    ...
    def test_custom_exclude_patterns(self, project_dir: Path) -> None:
        ...
        mgr = CheckpointManager(
            project_dir,
            CheckpointConfig(exclude_patterns=["*.env", ".git/", ".autoship/"]),
        )
        mgr.initialize()
        cp = mgr.snapshot(iteration=0, message="initial")
        assert cp is not None
