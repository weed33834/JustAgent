...
def test_download_package_raises_on_subprocess_exception(tmp_path: Path) -> None:
    ...
    from autoship.core.package_verifier import PackageDownloadError, download_package
    ...
    with (
        patch("autoship.core.package_verifier.subprocess.run", side_effect=boom),
        patch("autoship.utils.hashing.pip_cmd", return_value=["pip"]),
...
def test_download_package_raises_on_nonzero_returncode(tmp_path: Path) -> None:
    ...
    with (
        patch("autoship.core.package_verifier.subprocess.run", return_value=fake_result),
        patch("autoship.utils.hashing.pip_cmd", return_value=["pip"]),
        pytest.raises(PackageDownloadError),
    ):
        download_package("pkg==1.0.0", out_dir)
...
def test_download_and_verify_returns_moved_file(tmp_path: Path) -> None:
    ...
    from autoship.core.package_verifier import download_and_verify
    ...
    with (
        patch("autoship.core.package_verifier.download_package", side_effect=fake_download),
        patch("autoship.core.package_verifier.verify_package"),
    ):
...
def test_download_and_verify_with_dest_dir_does_not_register_atexit(tmp_path: Path) -> None:
    ...
    with (
        patch("autoship.core.package_verifier.download_package", side_effect=fake_download),
        patch("autoship.core.package_verifier.verify_package"),
        patch("autoship.core.package_verifier.atexit.register", side_effect=_tracking_register),
    ):
        result = download_and_verify(
            "pkg==2.0.0",
            sha256_hex=None,
