"""Tests for :mod:`autoship.core.batch_ops`."""
...
from autoship.core.batch_ops import (
    BatchOperation,
    BatchResult,
    BatchRunner,
    BatchSummary,
...
class TestBatchSummary:
    ...
    def test_totals_computed_by_runner(self, tmp_path: Path) -> None:
        ...
        with patch(
            "autoship.core.batch_ops.subprocess.run",
...
class TestBatchRunnerPipeline:
    ...
    def test_pipeline_shells_to_autoship(self, tmp_path: Path) -> None:
        store = ProjectStore(store_path=tmp_path / "projects.json")
        _add_project(store, tmp_path, "proj")
        runner = BatchRunner(store)
        with patch(
