"""Tests for :mod:`autoship.core.scheduler`."""
...
class TestScheduleStore:
    ...
    def test_atomic_write_uses_tmp_then_rename(self, tmp_path: Path) -> None:
        ...
        store = ScheduleStore(store_path=path)
        with patch(
            "autoship.core.scheduler.atomic_write_text"
        ) as mock_write:
            store.add(_make_task(name="alpha"))
...
class TestScheduler:
    ...
    def test_run_due_runs_due_task(self, tmp_path: Path) -> None:
        ...
        with patch(
            "autoship.core.scheduler.subprocess.run",
...
class TestEdgeCases:
    ...
    def test_daemon_loop_stops_on_keyboard_interrupt(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("autoship.core.scheduler.time.sleep", side_effect=fake_sleep),
        ):
...
def test_module_exports() -> None:
    ...
    from autoship.core import scheduler as module
