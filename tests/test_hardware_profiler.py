...
from justagent.core.hardware_profiler import HardwareProfile, detect_hardware


def test_detect_hardware_recommends_tier_3() -> None:
    with (
        patch("justagent.core.hardware_profiler._cpu_count", return_value=16),
        patch("justagent.core.hardware_profiler._memory_gb", return_value=32.0),
        patch("justagent.core.hardware_profiler._has_gpu", return_value=True),
    ):
...
def test_detect_hardware_recommends_tier_2() -> None:
    with (
        patch("justagent.core.hardware_profiler._cpu_count", return_value=4),
        patch("justagent.core.hardware_profiler._memory_gb", return_value=8.0),
        patch("justagent.core.hardware_profiler._has_gpu", return_value=False),
    ):
        profile = detect_hardware()
...
def test_detect_hardware_recommends_tier_1() -> None:
    with (
        patch("justagent.core.hardware_profiler._cpu_count", return_value=2),
        patch("justagent.core.hardware_profiler._memory_gb", return_value=4.0),
        patch("justagent.core.hardware_profiler._has_gpu", return_value=False),
    ):
        profile = detect_hardware()
...
def test_memory_gb_fallback_from_proc(tmp_path, monkeypatch) -> None:
    from justagent.core.hardware_profiler import _memory_gb
...
def test_has_gpu_detects_nvidia_smi() -> None:
    from justagent.core.hardware_profiler import _has_gpu
...
def test_cgroup_cpu_count_v2_quota(tmp_path, monkeypatch) -> None:
    ...
    from justagent.core.hardware_profiler import _cgroup_cpu_count
...
def test_cgroup_memory_gb_v2(tmp_path, monkeypatch) -> None:
    ...
    from justagent.core.hardware_profiler import _cgroup_memory_gb
