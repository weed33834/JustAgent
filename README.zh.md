...
配置值中可以用 `${VAR}` 引用环境变量，运行时会自动展开。完整配置项参考 `.myagent.toml.example`。

## AI 后端

MyAgent 通过 [LiteLLM](https://github.com/BerriAI/litellm) 统一调用大模型，LiteLLM 支持的供应商全部开箱可用——OpenAI、Anthropic、Ollama、OpenRouter、Azure、vLLM、LM Studio、llama.cpp 等 100 多家。

在 `.myagent.toml` 里配置一个或多个后端：
...
| `myagent clean` | 格式化并 lint 源码 |
| `myagent verify` | 执行测试套件 |
| `myagent commit` | 生成并创建提交 |
| `myagent upload` | 构建并发布制品 |
| `myagent ship` | 按序执行全流程 |
| `myagent fix` | AI 辅助的代码修复建议 |
...
- [GitCode](https://gitcode.com/badhope/myagent) — 国内用户克隆更快
