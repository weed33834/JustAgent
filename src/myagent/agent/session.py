...
Sessions are saved to ``~/.myagent/sessions/`` as JSON files. Each
...
def default_store_dir() -> Path:
    """Return the default session store directory (``~/.myagent/sessions``)."""

    return Path.home() / ".myagent" / "sessions"
...
class Session:
    ...
    ``messages`` are serialized :class:`myagent.agent.runtime.Message`
...
def get_session_store(store_dir: Path | None = None) -> SessionStore:
    ...
    default ``~/.myagent/sessions`` directory is used.
    """
