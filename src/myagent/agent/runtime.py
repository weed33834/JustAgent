...
* :class:`LLMClient` is a thin wrapper around LiteLLM that supports
  ...
  :class:`myagent.adapters.model_gateway.ModelGateway` because that
...
* Abort is via :class:`asyncio.Event` (matching
  :class:`myagent.agent.tools.base.ToolContext.abort`).
...
class LLMClient:
    ...
    Decoupled from :class:`myagent.adapters.model_gateway.ModelGateway`
