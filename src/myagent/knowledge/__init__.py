"""Enterprise knowledge management module for the Omniagent platform.
...
* **RAG pipeline** — retrieval-augmented generation with source
  ...
  :mod:`myagent.adapters.model_gateway` for LLM calls.
...
* :mod:`myagent.knowledge.document` — document models, parsing, lifecycle.
* :mod:`myagent.knowledge.vector` — vector store, embeddings, similarity.
* :mod:`myagent.knowledge.graph` — knowledge graph, entity/relation extraction.
* :mod:`myagent.knowledge.rag` — RAG pipeline with citations.
* :mod:`myagent.knowledge.etl` — ETL pipeline for data ingestion.

Quick start::

    from myagent.knowledge import (
        DocumentParser,
        InMemoryVectorStore,
        RAGPipeline,
        HashingEmbedder,
...
from myagent.knowledge.etl import (
    APISource,
    DatabaseSource,
    ETLPipeline,
    ETLSource,
...
from myagent.knowledge.graph import (
    Entity,
    EntityType,
    KnowledgeGraph,
    Relation,
...
from myagent.knowledge.rag import (
    Citation,
    RAGAnswer,
    RAGPipeline,
    create_pipeline,
