...
* :class:`ResourceMonitor` — collect, evaluate, alert, report and (optionally)
  push load back into a :class:`~myagent.resources.registry.ResourceRegistry`
...
collection updates that resource's :class:`~myagent.resources.registry.ResourceLoad`
...
logger = logging.getLogger("myagent.resources")
...
class ResourceMonitor:
    ...
    def bind_resource(self, registry: Any, resource_id: str) -> None:
        ...
        :class:`~myagent.resources.registry.ResourceLoad` with the live CPU
    ...
    def _push_to_registry(self, snapshot: MetricSnapshot) -> None:
        ...
        if self._registry is None or not self._bound_resource_id:
            return
        try:
            from myagent.resources.registry import ResourceLoad  # local import
    ...
    def start(self, interval: float | None = None) -> None:
        ...
        with self._lock:
            ...
            self._thread = threading.Thread(
                target=self._loop, name="myagent-monitor", daemon=True
            )
            self._running = True
            self._thread.start()
