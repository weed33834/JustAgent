"""The ``myagent ui`` command — a textual TUI dashboard."""
...
def _project_summary(config: AppConfig, audit_logger: AuditLogger) -> list[str]:
    from myagent.core.language_rules import primary_language, rules_for_project
    from myagent.utils.project_detector import detect_project_type
...
def _build_app(config: AppConfig, audit_logger: AuditLogger) -> Any:
    ...
    actions: list[tuple[str, str, list[str]]] = [
        ("1", "clean", [sys.executable, "-m", "myagent", "clean", "--yes"]),
        ("2", "verify", [sys.executable, "-m", "myagent", "verify", verify_command]),
        ("3", "commit", [sys.executable, "-m", "myagent", "commit"]),
        ("4", "upload --dry-run", [sys.executable, "-m", "myagent", "upload", "--dry-run"]),
        ("5", "doctor", [sys.executable, "-m", "myagent", "doctor"]),
    ]
    ...
    class MyAgentDashboard(App[None]):
        ...
        def compose(self) -> ComposeResult:
            yield Static(f"  MyAgent dashboard — {config.project_root}", id="header")
