"""The ``myagent schedule`` command group: manage scheduled tasks.

Wraps :class:`myagent.core.scheduler.Scheduler` so users can configure
...
def list_tasks() -> None:
    ...
    if not tasks:
        typer.echo("No scheduled tasks. Use 'myagent schedule add' to create one.")
        return
