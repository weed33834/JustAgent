"""The ``myagent project`` command group: manage multiple local projects.

Projects are tracked in ``~/.myagent/projects.json`` via
:class:`myagent.core.project_store.ProjectStore`. The group exposes
...
from myagent.core.batch_ops import BatchOperation, BatchRunner
...
def list_projects(ctx: typer.Context) -> None:
    ...
    if not projects:
        typer.echo("No managed projects. Use 'myagent project add PATH' to add one.")
        return
