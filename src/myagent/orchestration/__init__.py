"""Decision execution and orchestration module for the Omniagent platform.
...
* **Workflow** (:mod:`myagent.orchestration.workflow`) — a DAG-based workflow
...
* **Decision** (:mod:`myagent.orchestration.decision`) — a decision execution
...
* **Mesh** (:mod:`myagent.orchestration.mesh`) — an agent mesh networking
...
* **Coordinator** (:mod:`myagent.orchestration.coordinator`) — a multi-agent
...
Pydantic v2 for data models, and follow the ``myagent.orchestration.<submodule>``
logging namespace.
...
Quick start::
    ...
    from myagent.orchestration import (
...
logger = logging.getLogger("myagent.orchestration")
