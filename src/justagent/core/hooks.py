...
corresponding JustAgent command (``clean`` or ``verify``) in a subprocess.
...
* :func:`justagent.cli.commands.hooks` — the ``justagent hooks`` command
  (``list`` / ``run`` / ``watch``), the universal mechanism that works with
  any editor via filesystem events.
* :class:`justagent.cli.commands.lsp._LSPServer` — the ``textDocument/didSave``
...
class OnSaveHookRunner:
    ...
    def build_command(self, hook: HookConfig, path: Path) -> list[str] | None:
        ...
        """
        cmd: list[str] = [self.python_executable, "-m", "justagent", hook.command]
