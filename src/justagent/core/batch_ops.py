...
``justagent project batch-*`` commands.
"""
...
class BatchRunner:
    ...
    def run_pipeline(
        ...
        """Run workflow ``stages`` across projects by shelling out to ``justagent``.

        Each stage runs ``justagent <stage>`` in the project directory. Within
