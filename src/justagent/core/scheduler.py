...
Persist scheduled tasks to ``~/.justagent/schedules.json`` and run them
...
Used by the ``justagent schedule`` command group. The ``daemon`` mode
...
DEFAULT_SCHEDULE_STORE_PATH = Path.home() / ".justagent" / "schedules.json"
...
class ScheduleStore:
    ...
    :class:`justagent.core.project_store.ProjectStore`: missing or corrupt
