...
working tree into a hidden shadow git repo (``.justagent/checkpoints/``).
...
* :class:`CheckpointManager` owns the shadow repo. It initializes a bare
  git repo at ``<project>/.justagent/checkpoints/`` on first use, then
...
* The shadow repo is separate from the user's own git repo — it lives
  under ``.justagent/`` which is typically gitignored.
"""
...
class CheckpointConfig:
    ...
    Attributes:
        ...
        checkpoint_dir: Relative path under the project root for the
            shadow git repo. Defaults to ``.justagent/checkpoints``.
    ...
    enabled: bool = True
    checkpoint_dir: str = ".justagent/checkpoints"
...
class CheckpointManager:
    ...
    *content* of the project directory without touching the user's own
    ...
    def __init__(
        ...
        self._git_env = {
            "GIT_AUTHOR_NAME": "justagent-checkpoint",
            "GIT_AUTHOR_EMAIL": "checkpoint@justagent.local",
            "GIT_COMMITTER_NAME": "justagent-checkpoint",
            "GIT_COMMITTER_EMAIL": "checkpoint@justagent.local",
        }
    ...
    def initialize(self) -> None:
        ...
        Creates ``.justagent/checkpoints/`` as a git repository. Safe to
    ...
    def _format_commit_message(
        ...
        *,
        ...
        :meth:`_parse_commit_message`. We use a ``JUSTAGENT-CP:`` prefix
    ...
    def _prune_old_checkpoints(self) -> None:
        ...
        if cutoff < len(commits):
            ...
            self._run_git(
                ...
                "--quiet",
                "-m",
                "JUSTAGENT-CP: pruned history (root)",
                cwd=self._shadow_repo,
            )
