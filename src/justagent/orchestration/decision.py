...
logger = logging.getLogger("justagent.orchestration.decision")
...
class DecisionExecutor:
    ...
    async def _default_notify_handler(self, action: DecisionAction) -> dict[str, Any]:
        """Notify via :mod:`justagent.communication.notification` (lazy import)."""

        message = action.parameters.get("message", "")
        audience = action.target or action.parameters.get("audience", "all")
        try:
            from justagent.communication.notification import (  # lazy import
                NotificationEngine,
    ...
    async def _default_schedule_handler(self, action: DecisionAction) -> dict[str, Any]:
        """Schedule a meeting via :mod:`justagent.communication.meeting` (lazy)."""

        time_str = action.parameters.get("time", "")
        attendees = action.target or action.parameters.get("attendees", "")
        try:
            from justagent.communication.meeting import MeetingService  # lazy import
