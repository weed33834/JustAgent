"""JustAgent Agent — local-first AI agent kernel.
...
imported without the rest of the JustAgent CLI.

Sub-modules:

* :mod:`justagent.agent.patch` — ``apply_patch`` parser & applier
  (ported from Cline ``apply-patch-parser.ts`` / OpenCode ``patch/index.ts``).
* :mod:`justagent.agent.search_replace` — Aider's SEARCH/REPLACE block
  format with perfect / whitespace-tolerant / ``...``-elision / fuzzy
  fallback matching.
* :mod:`justagent.agent.loop_detection` — repeated-tool-call loop
  detection (ported from Cline ``loop-detection.ts``).
* :mod:`justagent.agent.mistake_tracker` — consecutive-mistake tracker
  with limit callbacks (ported from Cline ``mistake-tracker.ts``).
* :mod:`justagent.agent.tools` — Tool base class + registry +
...
from justagent.agent.tools import (
    Attachment,
    ExecuteFn,
