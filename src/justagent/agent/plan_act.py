...
to justagent. In **Plan mode** the agent explores and proposes a plan
...
class ModeSwitchTracker:
    ...
    justagent (unlike Cline's ``switch_to_act_mode`` tool).
...
def default_plan_file_path(cwd: str | Path) -> Path:
    ...
    ``.justagent/plans/plan.md`` within the project root. The directory
    is *not* created here — callers should ``mkdir(parents=True,
    exist_ok=True)`` before writing.
    """

    return Path(cwd) / ".justagent" / "plans" / "plan.md"
