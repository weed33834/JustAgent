"""The ``justagent agent`` command — run the local-first AI agent loop.

Wraps :class:`justagent.agent.runtime.AgentRuntime` in a Typer command so
users can drive the agent from the CLI:

* ``justagent agent "list files in this dir"`` — default Act mode, tools
  available, prompts before destructive ops (or ``--yes`` to skip).
* ``justagent agent --plan "explore the codebase and propose a refactor"``
  — Plan mode: edit tools hidden, read-only investigation only.
* ``justagent agent --yolo "fix the failing tests"`` — Yolo mode: all
  tools, no permission prompts.
* ``justagent agent --json "..."`` — emit NDJSON events on stdout for
...
3. ``config.llm`` (legacy single-backend section, also bridged by
   :func:`justagent.cli.commands.fix._model_router`).
"""
...
#: software engineer..." preamble, trimmed for justagent's scope.
DEFAULT_SYSTEM_PROMPT = (
    "You are justagent's built-in agent — a highly skilled software "
...
def _resolve_llm(
    ...
    *,
    ...
    if config.model.backends:
        ...
        if not model:
            raise typer.BadParameter(
                "Model backend has no `model` field — pass --model or fix .justagent.toml"
            )
        return LLMClient(
            model=model,
            api_key=api_key,
    ...
    if not api_key and provider is not Provider.OLLAMA:
        raise typer.BadParameter(
            "No API key configured — pass --api-key or set [llm].api_key in .justagent.toml"
        )
    return LLMClient(
        model=model,
        api_key=api_key,
...
def agent(
    ...
    一次性模式：``justagent agent "task"``
    交互模式：``justagent agent -i`` 或 ``justagent agent -i "initial task"``
    恢复模式：``justagent agent -i --resume <SESSION_ID>``
    """
    ...
    # later resumed via ``justagent session resume <ID>``.
