"""The ``justagent lsp`` command: a minimal LSP server over stdio.

Exposes JustAgent's ``verify`` diagnostics to any LSP-capable editor
...
Diagnostics source
------------------
The diagnostic payload comes from running ``justagent verify`` on the
...
never invent diagnostics that JustAgent itself did not produce.

Run from an editor with ``justagent lsp`` as the server command. JustAgent
does not need to be on ``$PATH``; the command can be invoked as
``python -m justagent lsp`` from a checkout.
"""
...
from justagent import __version__
...
logger = logging.getLogger("justagent")
...
def _run_verify(
    ...
    command is validated against the ``justagent verify`` subcommand's
    ...
    from justagent.cli.commands.verify import validate_verify_command
            ...
            return [
                {
                    ...
                    "message": f"justagent config load failed: {exc}",
                }
            ]
        ...
        return [
            {
                ...
                "message": (
                    ...
                    "update [verify] allowed_commands in .justagent.toml"
                ),
            }
        ]
...
class _LSPServer:
    ...
    def __init__(self, project_root: Path) -> None:
        ...
        # ``.justagent.toml`` are picked up without restarting the server.
        self._config: Any = None

    def _get_config(self) -> Any:
        """Lazily load and cache the project's :class:`AppConfig`.

        Returning the cached value avoids re-reading ``.justagent.toml`` on
    ...
    def _get_runner(self) -> Any:
        ...
        # Local import keeps the LSP module importable without the full
        # config/audit stack at module load time.
        from justagent.core.hooks import OnSaveHookRunner
    ...
    def handle(self, request: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if method == "initialize":
            return self._response(
                request_id,
                {
                    ...
                    "serverInfo": {"name": "justagent-lsp", "version": __version__},
                },
            )
        ...
        if method == "textDocument/didSave":
            ...
            # diagnostics pass) pick up any ``.justagent.toml`` changes the
...
def lsp(
    ...
    """Run the JustAgent LSP server over stdio."""
