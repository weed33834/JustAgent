...
from justagent.core.hardware_profiler import detect_hardware
from justagent.models.config import AppConfig
from justagent.plugin_manager import manager as plugin_manager
from justagent.utils.project_detector import PROJECT_MARKERS, detect_project_type


def _render_default_config(project_type: str, default_tier: int = 2) -> str:
    """Render a default ``.justagent.toml`` for the given project type."""
    return f'''# JustAgent configuration
schema_version = 1
...
def init(
    ...
    output: Path = typer.Option(Path(".justagent.toml"), "--output", "-o", help="Config file path"),
    yes: bool = typer.Option(False, "--yes", "-y", help="Skip interactive confirmations"),
) -> None:
    """Initialize an JustAgent configuration file for the current project."""
