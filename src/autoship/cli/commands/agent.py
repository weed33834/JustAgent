...
from autoship.agent.session import Session, SessionError, get_session_store
from autoship.agent.slash_commands import CommandAction, create_default_registry
...
from autoship.permissions import (
    PermissionAction,
    PermissionEngine,
    PermissionScope,
    ...
    while True:
        try:
            if json_mode:
                ...
                typer.secho(
                    "\nautoship> ", fg=typer.colors.CYAN, bold=True, nl=False
                )
                line = input()
...
def agent(
    ...
    一次性模式：``autoship agent "task"``
    交互模式：``autoship agent -i`` 或 ``autoship agent -i "initial task"``
    恢复模式：``autoship agent -i --resume <SESSION_ID>``
    """
    ...
    # later resumed via ``autoship session resume <ID>``.
