"""The ``autoship artifacts`` command: language-aware build output removal.
...
Rules live in :mod:`autoship.core.language_rules`; this command is a thin
...
Examples::
    ...
    autoship artifacts --dry-run       # show what would be removed
    autoship artifacts                 # remove build outputs
    autoship artifacts --list          # show which rules apply, no scanning
    autoship artifacts --language go   # restrict to one language
"""
...
from autoship.core.language_rules import (
    RULES,
...
def artifacts(
    ...
    if plan.total == 0:
        typer.echo("artifacts.nothing_to_remove")
        plugin_manager.call("post_clean", context=context, fail_fast=False)
        raise typer.Exit(code=0)

    typer.echo("artifacts.plan_header")
    ...
    if dry_run:
        typer.echo("artifacts.dry_run_summary")
        plugin_manager.call("post_clean", context=context, fail_fast=False)
        raise typer.Exit(code=0)

    skip_confirm = yes or ctx.obj.get("yes", False)
    if not skip_confirm and not typer.confirm("artifacts.confirm"):
        typer.echo("artifacts.aborted")
