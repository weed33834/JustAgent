"""The ``autoship schedule`` command group: manage scheduled tasks.

Wraps :class:`autoship.core.scheduler.Scheduler` so users can configure
...
from autoship.core.scheduler import (
    Scheduler,
    SchedulerError,
    ScheduleStore,
)
...
def list_tasks() -> None:
    ...
    if not tasks:
        typer.echo("No scheduled tasks. Use 'autoship schedule add' to create one.")
        return
...
def run_task(
    ...
    except (SchedulerError, AutoShipError) as exc:
