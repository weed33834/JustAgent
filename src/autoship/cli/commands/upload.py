...
def register(parent: typer.Typer) -> None:
    parent.command(name="upload", help="upload.help")(upload)


@app.command(name="upload")
def upload(
    ctx: typer.Context,
    target: str = typer.Option(..., "--target", help="upload.option.target"),
    image: str | None = typer.Option(None, "--image", help="upload.option.image"),
    tag: str | None = typer.Option(None, "--tag", "-t", help="upload.option.tag"),
    artifacts: list[str] | None = typer.Option(None, "--artifact", help="upload.option.artifact"),
    repository: str | None = typer.Option(None, "--repository", help="upload.option.repository"),
    repository_url: str | None = typer.Option(
        None, "--repository-url", help="upload.option.repository_url"
    ),
    registry: str | None = typer.Option(None, "--registry", help="upload.option.registry"),
    dry_run: bool = typer.Option(False, "--dry-run", "-n", help="upload.option.dry_run"),
) -> None:
    ...
    if repository_url:
        if not PyPIUploader.is_safe_repository_url(repository_url):
            raise UploadError("upload.repository_url_invalid")
    ...
    try:
        ...
        if dry_run:
            typer.echo("upload.dry_run_not_configured")
            audit.record("upload.dry_run_not_configured", {"target": target, "reason": str(exc)})
            return
        raise

    if not dry_run and not yes and not typer.confirm("upload.confirm"):
        typer.echo("upload.aborted")
    ...
    if dry_run:
        ...
        typer.echo("upload.dry_run")
        return

    audit.record("upload.done", {"target": target, "result": result.details})
    plugin_manager.call("post_upload", context=context, fail_fast=False)
    if result.url:
        typer.echo("upload.result_url")
    else:
        typer.echo("upload.result")
