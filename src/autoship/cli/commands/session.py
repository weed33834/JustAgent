"""``autoship session`` 命令：管理会话历史。

支持列出、查看、恢复、删除已保存的 agent 会话。会话存储在
``~/.autoship/sessions/`` 下（可通过 ``AUTOSHIP_SESSIONS_DIR`` 环境变量覆盖）。
...
def resume_session(
    ...
    typer.echo(f"  autoship agent -i --resume {session_id}")
