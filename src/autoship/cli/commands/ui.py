"""The ``autoship ui`` command — a textual TUI dashboard."""
...
def _project_summary(config: AppConfig, audit_logger: AuditLogger) -> list[str]:
    from autoship.core.language_rules import primary_language, rules_for_project
...
def _build_app(config: AppConfig, audit_logger: AuditLogger) -> Any:
    ...
    actions: list[tuple[str, str, list[str]]] = [
        ("1", "clean", [sys.executable, "-m", "autoship", "clean", "--yes"]),
        ("2", "verify", [sys.executable, "-m", "autoship", "verify", verify_command]),
        ("3", "commit", [sys.executable, "-m", "autoship", "commit"]),
        ("4", "upload --dry-run", [sys.executable, "-m", "autoship", "upload", "--dry-run"]),
        ("5", "doctor", [sys.executable, "-m", "autoship", "doctor"]),
    ]

    class AutoShipDashboard(App[None]):
        CSS = """
        ...
        def compose(self) -> ComposeResult:
            yield Static(f"  AutoShip dashboard — {config.project_root}", id="header")
