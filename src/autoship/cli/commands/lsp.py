"""The ``autoship lsp`` command: a minimal LSP server over stdio.

Exposes AutoShip's ``verify`` diagnostics to any LSP-capable editor
...
Diagnostics source
------------------
The diagnostic payload comes from running ``autoship verify`` on the
...
never invent diagnostics that AutoShip itself did not produce.

Run from an editor with ``autoship lsp`` as the server command. AutoShip
does not need to be on ``$PATH``; the command can be invoked as
``python -m autoship lsp`` from a checkout.
"""
...
def _run_verify(
    ...
    command is validated against the ``autoship verify`` subcommand's
...
class _LSPServer:
    ...
    def __init__(self, project_root: Path) -> None:
        ...
        # ``.autoship.toml`` are picked up without restarting the server.
        self._config: Any = None

    def _get_config(self) -> Any:
        """Lazily load and cache the project's :class:`AppConfig`.

        Returning the cached value avoids re-reading ``.autoship.toml`` on
    ...
    def handle(self, request: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if method == "initialize":
            return self._response(
                request_id,
                {
                    ...
                    "serverInfo": {"name": "autoship-lsp", "version": __version__},
                },
            )
        ...
        if method == "textDocument/didSave":
            ...
            # diagnostics pass) pick up any ``.autoship.toml`` changes the
