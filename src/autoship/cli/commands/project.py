"""The ``autoship project`` command group: manage multiple local projects.

Projects are tracked in ``~/.autoship/projects.json`` via
:class:`autoship.core.project_store.ProjectStore`. The group exposes
...
from autoship.core.batch_ops import BatchOperation, BatchRunner
from autoship.core.project_discovery import (
    DiscoveryConfig,
    ProjectDiscovery,
...
def list_projects(ctx: typer.Context) -> None:
    ...
    if not projects:
        typer.echo("No managed projects. Use 'autoship project add PATH' to add one.")
        return
...
def scan(
    ...
    try:
        if dry_run:
            ...
            return
    except (ProjectDiscoveryError, AutoShipError) as exc:
