...
def _resolve_directory_paths(config: AppConfig) -> list[Path]:
    ...
    cache_dir = Path.home() / ".autoship" / "cache"
...
def build_report() -> DoctorReport:
    ...
    try:
        ...
        report.add("config", Status.ERROR, "配置加载失败。", "运行 `autoship init` 创建配置。")
        return report
