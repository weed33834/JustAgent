...
Persist scheduled tasks to ``~/.autoship/schedules.json`` and run them
...
Used by the ``autoship schedule`` command group. The ``daemon`` mode
...
DEFAULT_SCHEDULE_STORE_PATH = Path.home() / ".autoship" / "schedules.json"
...
class SchedulerError(AutoShipError):
...
class ScheduleStore:
    ...
    :class:`autoship.core.project_store.ProjectStore`: missing or corrupt
