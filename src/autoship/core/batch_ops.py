...
``autoship project batch-*`` commands.
"""
...
class BatchOpsError(AutoShipError):
...
class BatchRunner:
    ...
    def run_pipeline(
        ...
        """Run pipeline ``stages`` across projects by shelling out to ``autoship``.

        Each stage runs ``autoship <stage>`` in the project directory. Within
