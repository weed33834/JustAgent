...
corresponding AutoShip command (``clean`` or ``verify``) in a subprocess.
...
* :func:`autoship.cli.commands.hooks` — the ``autoship hooks`` command
  (``list`` / ``run`` / ``watch``), the universal mechanism that works with
  any editor via filesystem events.
* :class:`autoship.cli.commands.lsp._LSPServer` — the ``textDocument/didSave``
...
if TYPE_CHECKING:
    ...
    from autoship.models.config import AppConfig, HookConfig
...
class OnSaveHookRunner:
    ...
    def build_command(self, hook: HookConfig, path: Path) -> list[str] | None:
        ...
        """
        cmd: list[str] = [self.python_executable, "-m", "autoship", hook.command]
