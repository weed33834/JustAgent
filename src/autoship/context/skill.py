...
under ``.autoship/skills/`` and contains a ``SKILL.md`` file with
...
class SkillError(AutoShipError):
...
class SkillLoader:
    ...
    one or more ``skills_dirs`` (default ``.autoship/skills``). Only the
