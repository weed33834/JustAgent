...
working tree into a hidden shadow git repo (``.autoship/checkpoints/``).
...
* :class:`CheckpointManager` owns the shadow repo. It initializes a bare
  git repo at ``<project>/.autoship/checkpoints/`` on first use, then
...
* The shadow repo is separate from the user's own git repo — it lives
  under ``.autoship/`` which is typically gitignored.
"""
...
class CheckpointError(AutoShipError):
...
class CheckpointConfig:
    ...
    Attributes:
        ...
        checkpoint_dir: Relative path under the project root for the
            shadow git repo. Defaults to ``.autoship/checkpoints``.
    ...
    enabled: bool = True
    checkpoint_dir: str = ".autoship/checkpoints"
...
class CheckpointManager:
    ...
    *content* of the project directory without touching the user's own
    ...
    def __init__(
        ...
        self._git_env = {
            "GIT_AUTHOR_NAME": "autoship-checkpoint",
            "GIT_AUTHOR_EMAIL": "checkpoint@autoship.local",
            "GIT_COMMITTER_NAME": "autoship-checkpoint",
            "GIT_COMMITTER_EMAIL": "checkpoint@autoship.local",
        }
    ...
    def initialize(self) -> None:
        ...
        Creates ``.autoship/checkpoints/`` as a git repository. Safe to
        ...
        self._run_git(
            "config", "user.name", "autoship-checkpoint", cwd=self._shadow_repo
        )
        self._run_git(
            "config", "user.email", "checkpoint@autoship.local", cwd=self._shadow_repo
        )
    ...
    def _format_commit_message(
        ...
        *,
        ...
        :meth:`_parse_commit_message`. We use a ``AUTOSHIP-CP:`` prefix
        ...
        return f"AUTOSHIP-CP: {json.dumps(envelope, ensure_ascii=False)}"
    ...
    def _prune_old_checkpoints(self) -> None:
        ...
        if cutoff < len(commits):
            ...
            self._run_git(
                ...
                "--quiet",
                "-m",
                "AUTOSHIP-CP: pruned history (root)",
                cwd=self._shadow_repo,
            )
