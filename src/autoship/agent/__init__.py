"""AutoShip Agent — local-first AI agent kernel.
...
imported without the rest of the AutoShip pipeline machinery.

Sub-modules:

* :mod:`autoship.agent.patch` — ``apply_patch`` parser & applier
  (ported from Cline ``apply-patch-parser.ts`` / OpenCode ``patch/index.ts``).
* :mod:`autoship.agent.search_replace` — Aider's SEARCH/REPLACE block
  format with perfect / whitespace-tolerant / ``...``-elision / fuzzy
  fallback matching.
* :mod:`autoship.agent.loop_detection` — repeated-tool-call loop
  detection (ported from Cline ``loop-detection.ts``).
* :mod:`autoship.agent.mistake_tracker` — consecutive-mistake tracker
  with limit callbacks (ported from Cline ``mistake-tracker.ts``).
* :mod:`autoship.agent.tools` — Tool base class + registry +
...
from autoship.agent.tools import (
    Attachment,
    ExecuteFn,
