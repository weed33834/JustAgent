...
Sessions are saved to ``~/.autoship/sessions/`` as JSON files. Each
...
from autoship.utils.atomic_write import atomic_write_text
...
def default_store_dir() -> Path:
    """Return the default session store directory (``~/.autoship/sessions``)."""

    return Path.home() / ".autoship" / "sessions"
...
class SessionError(AutoShipError):
...
class Session:
    ...
    ``messages`` are serialized :class:`autoship.agent.runtime.Message`
...
def get_session_store(store_dir: Path | None = None) -> SessionStore:
    """Build a :class:`SessionStore`, honoring the ``AUTOSHIP_SESSIONS_DIR`` env var.

    Explicit ``store_dir`` wins; otherwise the ``AUTOSHIP_SESSIONS_DIR``
    environment variable is consulted (used by tests); otherwise the
    default ``~/.autoship/sessions`` directory is used.
    """

    if store_dir is not None:
        return SessionStore(store_dir)
    env_dir = os.environ.get("AUTOSHIP_SESSIONS_DIR")
    if env_dir:
